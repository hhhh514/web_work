...
def generate_secure_token(length=32):
    ...
    return secure_token
miss_new = generate_secure_token()
def verify_token(token):
    return token == miss_new
@app.after_request
...
def get_product(id):
    ...
    return jsonify({
        ...
        'reviews': [{
            ...
            'customer_name': Customer.query.get(r.customer_id).name if Customer.query.get(r.customer_id) else 'Unknown',
            'rating': r.rating,
            'comment': r.comment,
...
@app.route('/products/seller/<int:seller_id>', methods=['GET'])
...
def login():
    ...
    return jsonify({
        ...
        'id': user.customer_id if role == 'customer' else user.seller_id,
        'token': miss_new
        })
...
@app.route('/sellers/<int:id>/rating', methods=['PATCH'])
def update_seller_rating(id):
...
def get_customer_orders(customer_id):
    orders = OrderTable.query.filter_by(customer_id=customer_id).all()
    result = []
    for order in orders:
        order_data = {
    ...
    return jsonify(result)
@app.route('/orders/seller/<int:seller_id>', methods=['GET'])
def get_seller_orders(seller_id):
    items = OrderItem.query.filter_by(seller_id=seller_id).all()
    result = []
    for item in items:
        result.append({
            'order_item_id': item.order_item_id,
...
def add_cart_item(cart_id):
    ...
    db.session.add(cart_item)
    cart.last_updated = datetime.now(UTC)
    db.session.commit()
    return jsonify({'message': 'Item added to cart'}), 201

@app.route('/carts/<int:customer_id>', methods=['GET'])
def get_cart(customer_id):
    ...
    general_cart = Cart.query.filter_by(customer_id=customer_id, is_category=0).first()
    ...
    categorized_carts = Cart.query.filter_by(customer_id=customer_id, is_category=1).all()
...
def get_wishlist(customer_id):
    wishlist = Wishlist.query.filter_by(customer_id=customer_id).first_or_404()
    return jsonify({
...
@app.route('/wishlists/customer/<int:customer_id>/items', methods=['POST'])
def add_item_by_customer_id(customer_id):
    data = request.get_json()
    product_id = data.get('product_id')
    if not product_id:
...
# 銷售總結相關 API
@app.route('/sales-summary/seller/<int:seller_id>', methods=['GET'])
def get_seller_sales_summary(seller_id):
    summaries = SalesSummary.query.filter_by(seller_id=seller_id).all()
    return jsonify([{
...
def create_product():
    if g.role != 'seller':
        return jsonify({'error': 'Seller only'}), 403
